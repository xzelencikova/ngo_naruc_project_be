...
class ClientsService:
    ...
    def create_new_client(self, payload):
        try:
            ...
            self.repo._load_data_to_db(
                table_name="clients", df=self.repo._convert_to_dataframe(payload)
            )
            return {"message": "The client was successfully created.", "status": 200}
    ...
    def set_client_active_status(self, payload):
        try:
            lock_clients = ", ".join([str(id) for id in payload["lock_clients"]])
            unlock_clients = ", ".join([str(id) for id in payload["unlock_clients"]])

            self.repo.set_client_active_status(clients=lock_clients, active=False)
            self.repo.set_client_active_status(clients=unlock_clients, active=True)
            return {
    ...
    def delete_client_by_id(self, id):
        try:
            ...
            self.repo._commit(self.repo.delete_client_by_id(id))
            return {
